...
class ParseDataFromTMBD {
  ...
  static async getInitDataBase(req, res) {
    try {
      await ParseDataFromTMBD.getGenres(req, res, false);
      await ParseDataFromTMBD.getLanguages(req, res, false);
      await ParseDataFromTMBD.getFilms(req, res, false);
  ...
  static async getGenres(req, res, isNeedResponse = true) {
    try {
      ...
      if (genresData.genres && genresData.genres.length) {
        ...
        await pgClient.query(`INSERT INTO genres (name, tmbd_genre_id) VALUES ${genresNames}`);
      } else {
  ...
  static async getLanguages(req, res, isNeedResponse = true) {
    try {
      ...
      if (languages && languages.length) {
        ...
        await pgClient.query(`INSERT INTO languages_tmbd 
  ...
  static async getFilms(req, res, isNeedResponse = true) {
    try {
      for (let i = 1; i <= 5; i++) {
        const moviesByPage = await getFuncForGetDataTMBD('/movie/top_rated', { page: i });
        for (const el of moviesByPage.results) {
          try {
            const movie = await getFuncForGetDataTMBD(`/movie/${el.id}`);
            const id = await pgClient.query(`INSERT INTO movie
                            (adult,
                             backdrop_path,
                             budget,
                             homepage,
                             ...
                             VALUES (${movie.adult || false},
                              ...
                              RETURNING id`);
            if (el.genre_ids) {
              for (const gnr of el.genre_ids) {
                const genreId = await pgClient.query(`SELECT id FROM genres WHERE tmbd_genre_id=${gnr}`);
                if (genreId.rows.length) {
                  await pgClient.query(`INSERT INTO movie_genres (movie_id, genre_id) VALUES (${id.rows[0].id}, ${genreId.rows[0].id})`);
                }
              }
            }
          } catch (e) {
